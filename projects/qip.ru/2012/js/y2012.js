...
$(function(){
	$('dl.qip2012-nav-list dt').hover(function(){
		var el = $(this);
		menuItem = el;
...
//			new_move.animate({top: 0}, 'slow');
	...
	function count() {
		...
		if(time_delta <= 0) {
			container.replaceWith('<div class="qip2012-congratulation"></div>');
			return;
		}
...
function giftsHandler(){
	$('div.gift').hover(function(){
		...
		var wish = $(this);
		if(!(wish.attr('title'))){
			$.get('y2012/getWish/0/'+jid+'/'+wish_id, function(data) {
	...
	$('div.gift').click(function(){
		...
		if(!jid || !wish_id) return;
		var wish = $(this);
		$.get('y2012/getWish/1/'+jid+'/'+wish_id, function(data) {
